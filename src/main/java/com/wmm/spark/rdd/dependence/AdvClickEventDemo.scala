package com.wmm.spark.rdd.dependence
...
object AdvClickEventDemo extends App {
  ...
  println("logTextRDD : " + logTextRDD.toDebugString)
  logTextRDD.dependencies.foreach(println)

  val advClickEventRDD: RDD[((String, String), Int)] = logTextRDD.map(log => {
    val logArr: Array[String] = log.split(" ")
    ((logArr(1), logArr(4)), 1)
  })
  println("advClickEventRDD : " + advClickEventRDD.toDebugString)
  advClickEventRDD.dependencies.foreach(println)

  val reduceByKeyRDD: RDD[((String, String), Int)] = advClickEventRDD.reduceByKey(_ + _)
  println("reduceByKeyRDD : " + reduceByKeyRDD.toDebugString)
  reduceByKeyRDD.dependencies.foreach(println)
  val mapRDD: RDD[(String, (String, Int))] = reduceByKeyRDD.map((tuple: ((String, String), Int)) => (tuple._1._1, (tuple._1._2, tuple._2)))
  println("mapRDD : " + mapRDD.toDebugString)
  mapRDD.dependencies.foreach(println)
  val groupByKeyRDD: RDD[(String, Iterable[(String, Int)])] = mapRDD.groupByKey()
  println("groupByKeyRDD : " + groupByKeyRDD.toDebugString)
  groupByKeyRDD.dependencies.foreach(println)
